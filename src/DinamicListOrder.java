...
public class DinamicListOrder<T extends Comparable<T>> {
    private No<T> first;
    private No<T> last;
    private int cont;

    public DinamicListOrder() {
        first = null;
        last = null;
        cont = 0;
    }
    ...
    public void add(T elem){
        ...
        if (first == null){
            first = novo;
            last = novo;
        } else if (novo.data.compareTo(first.data) < 0) {
            novo.next = first;
            first = novo;
        } else if (novo.data.compareTo(last.data) >= 0) {
            last.next = novo;
            last = novo;
        } else {
            No<T> aux = first;
            No<T> before = null;
            while (aux != null){
                if (aux.data.compareTo(novo.data) <= 0){
                    before = aux;
                }
                aux = aux.next;
            }
